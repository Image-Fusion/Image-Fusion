...
def RF(f, p, q): # Row Frequency
    ...
    for i in range(0, f.shape[0]):
        for j in range(0, f.shape[1]):
            count = 0.0
            r = 0
            for x in range(max(i - int(p/2), 0), min(i + int(p/2) + 1, f.shape[0])): # p should be odd
                for y in range(max(j - int(q/2) + 1, 0), min(j + int(p/2) + 1, f.shape[1])): # q should be odd to make patch of p*q
                    count = count + 1
                    r = r + (f[x][y] - f[x][y-1])**2
            rf[i][j] = (r/count)**0.5
    return rf
...
def CF(f, p, q): # Column Frequency
    ...
    for i in range(0, f.shape[0]):
        for j in range(0, f.shape[1]):
            count = 0.0
            c = 0
            for x in range(max(i - int(p/2) + 1, 0), min(i + int(p/2) + 1, f.shape[0])): # p should be odd
                for y in range(max(j - int(q/2), 0), min(j + int(p/2) + 1, f.shape[1])): # q should be odd to make patch of p*q
                    count = count + 1
                    c = c + (f[x][y] - f[x - 1][y])**2
            cf[i][j] = (c/count) ** 0.5
    return cf
